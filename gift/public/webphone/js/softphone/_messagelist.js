...
function onCreate (event) // called only once - bind events here
{
    ...
    $('#messagelist_list').on('click', 'li', function(event)
    {
        OnListItemClick($(this).attr('id'));
    });
    
    $('#messagelist_notification_list').on('click', '.nt_anchor', function(event)
    {
        $("#messagelist_not").panel( "close" );
        common.NotificationOnClick2($(this).attr('id'), false);
    });
    $('#messagelist_notification_list').on('click', '.nt_menu', function(event)
    {
        $("#messagelist_not").panel( "close" );
        common.NotificationOnClick2($(this).attr('id'), true);
    });
    
    $("#messagelist_not_btn").on("click", function()
    {
    ...
    {
        if ($.mobile.activePage.attr('id') === 'page_messagelist')
        {
            MeasureMessagelist();
        }
    });
    
    $('#messagelist_menu_ul').on('click', 'li', function(event)
    {
        MenuItemSelected($(this).attr('id'));
    });
    $("#btn_messagelist_menu").on("click", function() { CreateOptionsMenu('#messagelist_menu_ul'); });
    $("#btn_messagelist_menu").attr("title", stringres.get("hint_menu"));
    

    $("#msglist_btnback").attr("title", stringres.get("hint_btnback"));

    $("#btn_newmessage").on("click", function() { NewMessage(); });
...
function onStart(event)
{
    ...
    $('#btn_newmessage').html(stringres.get('btn_new_message'));
    
    $(".separator_line_thick").css( 'background-color', common.HoverCalc(common.getBgColor('#page_messagelist'), -30) );
    ...
    $("#msglist_title").attr("title", stringres.get("hint_page"));

    if (!common.isNull(document.getElementById('msglist_btnback')))
    {
        document.getElementById('msglist_btnback').innerHTML = '<span>&LT;</span>&nbsp;' + stringres.get("go_back_btn_txt");
    }
...
// needed for proper display and scrolling of listview
    ...
    if (common.IsIeVersion(10)) { $("#messagelist_notification_list").children().css('line-height', 'normal'); }
    $("#messagelist_notification_list").height(common.GetDeviceHeight() - 55);
...
{
    try{
    //var pgh = common.GetDeviceHeight() - 1; $('#page_messagelist').css('min-height', pgh + 'px'); // must be set when softphone is skin in div
    $('#page_messagelist').css('min-height', 'auto'); // must be set when softphone is skin in div
    
// handle notifiaction      additional_header_right
    var notwidth = common.GetDeviceWidth() - $("#messagelist_additional_header_left").width() - $("#messagelist_additional_header_right").width();
    var margin = common.StrToIntPx( $("#messagelist_additional_header_left").css("margin-left") );
...
// handle page height
    var heightTemp = common.GetDeviceHeight() - $("#messagelist_header").height() - $("#btn_newmessage_container").height();

    heightTemp = Math.floor( heightTemp - 3 );
    $("#messagelist_list").height(heightTemp);
...
// check if there are missed messages
    ...
    $('#messagelist_list').html('');
    $('#messagelist_list').append(listview).listview('refresh');
...
function OnListItemClick (id) // :no return value
{
    ...
    var to = $('#msgitemnumber_' + msgid).html();
    if (common.isNull(to)) { to = ''; }else{ to = common.Trim(to); }
    
    var typestr = $('#msgtype_' + msgid).html();
    ...
    {
        $.mobile.changePage("#page_message", { transition: "none", role: "page" });    
    }else
    {
        $.mobile.changePage("#page_message", { transition: "slide", role: "page" });    
    }
...
// remove data transition for windows softphone, because it's slow
    ...
    {
        $( "#btn_messagelist_menu" ).removeAttr('data-transition');
    }

    if ( common.isNull(menuId) || menuId.lenght < 1 ) { common.PutToDebugLog(2, "ERROR, _messagelist: CreateOptionsMenu menuid null"); return; }

    if ($(menuId).length <= 0) { common.PutToDebugLog(2, "ERROR, _messagelist: CreateOptionsMenu can't get reference to Menu"); return; }
    
    if (menuId.charAt(0) !== '#') { menuId = '#' + menuId; }
    
    $(menuId).html('');
    $(menuId).append( '<li id="' + MENUITEM_MESSAGELIST_NEWMESSAGE + '"><a data-rel="back">' + stringres.get('btn_new_message') + '</a></li>' ).listview('refresh');
    
    $(menuId).append( '<li id="' + MENUITEM_MESSAGELIST_DELETE + '"><a data-rel="back">' + stringres.get('delete_text') + '</a></li>' ).listview('refresh');
...
function MenuItemSelected(itemid)
{
    ...
    $( '#messagelist_menu' ).on( 'popupafterclose', function( event )
    {
        $( '#messagelist_menu' ).off( 'popupafterclose' );
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _messagelist: onDestroy");
    global.isMessagelistStarted = false;

    } catch(err) { common.PutToDebugLogException(2, "_messagelist: onDestroy", err); }
}
