...
/*
    ...
    global.File.DeleteFile(filename, function (content)
    {
        alert('Deleted');
    });
*/

    //common.EngineSelect(0);
...
{
    //file.GetItem2('somekey', function(output) { alert('somekey = ' + output); } );
    try{
/*
    setTimeout(function ()
    {
        common.IsJavaInstalled();
    },200);*/
                                
//    setTimeout(function () { common.EngineForcedInstall('ns'); }, 500);
...
// content was hidden untill jquery mobile loaded. now dislpay content
    ...
    // detect IE version, if lower then 7, prompt user to use a higher version, redirect to download page - or skip
    // this application requires a higher version of internet explorer: Download / Skip
    /*if (common.IsIeVersion(6) || common.IsIeVersion(7))
    {
        var ret = confirm(stringres.get('ie_update'));
        if (ret === true)
        {
...
// login page with username and password
    ...
    $("#lp_btn_login").on("click", function()
    {
        ...
        if (!common.isNull(srv_field) && $('#lp_serveraddress').closest( 'div.ui-input-text' ).is(':visible') == true)
        {
            ...
            lpsrv = lpsrv.replace('udp://', ''); lpsrv = lpsrv.replace('tcp://', '');
            lpsrv = lpsrv.replace('wss://', ''); lpsrv = lpsrv.replace('ws://', '');
    ...
    if (!common.isNull(logo) && logo.length > 2 && !common.isNull(logodiv))
    {
        ...
        if (isimg)
        {
            logodiv.innerHTML = '<img src="images/' + logo + '" style="border: 0;">';
        }else
        {
...
function OfferNativeEngine(popupafterclose)
{
    ...
    var message = '<a href="#" id="pusenative">' + stringres.get('usenative_option_native') + '</a><br /><br />';
    message = message + '<a href="#" id="pusewebrtc">' + stringres.get('usenative_option_webrtc') + '</a>';
...
'</div>';
    ...
    {
        global.flagrestartwebrtc = false;
        $.mobile.activePage.find(".messagePopup").popup( "close" );
...
//DBG                 common.PutToDebugLog(3, 'DEBUG, _settings GetAccounts callback no content_2');
                common.GetSettingsFromUrl();
                common.GetOverWriteSettings();
...
var settOrderWebphone = 'serveraddress_user,sipusername,password,submenu_sipsettings,submenu_media,submenu_calldivert,submenu_general,theme,username,displayname,email,hidemyidentity,usetunneling,transport,proxyaddress,realm,use_fast_ice,use_stun,use_rport,register,registerinterval,filters,keepalive,natopenpackets,ringtimeout,calltimeout,dtmfmode,chatsms,prack,sendearlymedia,sendrtponmuted,defmute,changesptoring,localip,signalingport,rtpport,capabilityrequest,customsipheader,techprefix,callforwardonbusy,callforwardalways,calltransferalways,autoignore,autoaccept,transfwithreplace,voicemailnum,callbacknumber,blacklist,transfertype,automute,autohold,normalizenumber,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,audiodevices,displayvolumecontrols,displayaudiodevice,savetocontacts,sendchatonenter,extraoption,reset_settings,loglevel,loglevel_dbg,playring,'+
        'video,video_bandwidth,video_width,video_height,'+
        'cfgvideo,video_profile,use_h263,use_h264,video_fps,codec,setfinalcodec,aec,denoise,agc,plc,silencesupress,hardwaremedia,volumein,volumeout,mediaencryption,setqos,codecframecount,doublesendrtp,jittersize,audiobufferlength,speakermode';
var settOrderWebphoneWebRTCFlash = 'serveraddress_user,sipusername,password,submenu_sipsettings,submenu_media,submenu_calldivert,submenu_general,theme,username,displayname,email,hidemyidentity,usetunneling,realm,filters,ringtimeout,calltimeout,dtmfmode,chatsms,defmute,customsipheader,techprefix,voicemailnum,callforwardonbusy,callforwardalways,calltransferalways,autoignore,autoaccept,transfwithreplace,callbacknumber,blacklist,normalizenumber,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,audiodevices,displayvolumecontrols,displayaudiodevice,savetocontacts,sendchatonenter,extraoption,reset_settings,loglevel,loglevel_dbg,'+
        'video,video_bandwidth,video_width,video_height,'+
        'cfgvideo,video_profile,use_h263,use_h264,video_fps,aec,agc,volumein';
...
var basicSettings = 'settobasefunctionality,theme,username,displayname,usetunneling,transport,proxyaddress,register,ringtone,filters,callforwardonbusy,callforwardalways,calltransferalways,autoaccept,audiodevices,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,email,cpualwayspartiallock,forcewifi,accounts,loglevel,startwithos';
var basicSettingsCustomized = 'settobasefunctionality,theme,displayname,ringtone,filters,callforwardonbusy,callforwardalways,calltransferalways,autoaccept,audiodevices,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,email,cpualwayspartiallock,forcewifi,accounts,loglevel,startwithos';
...
// put username / password in login fields
    ...
    while (settOrderTmp.length > 0 && loopindex < 5000)
    {
        ...
        var value = common.settmap[settName];
...
// dialer integration not implemented
        ...
        if (settName === 'theme' && common.GetConfig('showtheme') === 'false') { continue; }
        
        if (settName === 'loglevel_dbg')
        {
            var loglevel = common.GetParameterInt('loglevel', 1);
            if (loglevel < 2)
            {
                continue;
            }
...
// if demo index page, then also read settings from cookie
        try{
        if (window.location.href.indexOf('iswponlinedemo=true') > 0
...
//<li data-icon="false"><a href="javascript:void(0)" class="noshadow"><div class="sett_text"><span class="sett_display_name">Password</span><br><span class="sett_comment">Sip account password Sip account password Sip account password</span></div><div class="sett_image"><img src="images\checkbox_true.png" /></div></a></li>
        ...
        if (settType === '0')
        {
            if (settName === 'use_h263' || settName === 'use_h264') // use_h263: 1,2; use_h264: 1,3
            {
                if (settValue === '1')
                {
                    listitem = listitem.replace('[TRUEFALSEIMG]', '<div class="sett_image"><img src="images/checkbox_false.png" id="img_' + settName + '" /></div>');
                }else
                {
                    listitem = listitem.replace('[TRUEFALSEIMG]', '<div class="sett_image"><img src="images/checkbox_true.png" id="img_' + settName + '" /></div>');
                }
            }else
            {
...
// choose  engine for testing !!!DEPRECATED
    if (common.StrToInt(common.GROUP_LOGIN))
    {
        ...
        {
            engine = engine.replace('[TRUEFALSEIMG]', '<div class="sett_image"><img src="images/checkbox_true.png" id="img_' + settName + '" /></div>');
        }else
        {
            engine = engine.replace('[TRUEFALSEIMG]', '<div class="sett_image"><img src="images/checkbox_false.png" id="img_' + settName + '" /></div>');
        }
...
}else
{
    ...
    {
        listview = listview + '<a href="#" id="al_newuser" class="settings_links" target="_blank">' + stringres.get('newuser') + '</a>';
...
//    listElem.listview('refresh');
    //$(menuId).append('<li><a href="#" id="' + MENUITEM_SETTINGS_EXIT + '" onclick="MenuItemSelected(\'' + MENUITEM_SETTINGS_EXIT + '\')" >Exit</a></li>').listview('refresh');
}
...
'<div id="settings_user_ce_select" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + pWidth + 'px; min-width: ' + Math.floor(pWidth * 0.6) + 'px;">' +
    ...
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="#" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_ok') + '</a>' +
        '<a href="#" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back">' + stringres.get('btn_cancel') + '</a>' +
    '</div>' +
'</div>';
...
/* !!!DEPRECATED    
    ...
    var value = common.settmap[mCurrSettName];
...
//'</form>' +
    ...
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
    '<a href="#" style="width: 98%;" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_ok') + '</a>' +
    '</div>' +
'</div>';
...
'<div id="settings_type_server" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + popupWidth + 'px;">' +
    ...
    '<div role="main" class="ui-content adialog_content adialog_btn_nexttoinput">' +
        ...
        '<input type="text" id="setting_item_input" name="setting_item" data-theme="a"/>' +
        '<button id="btn_srvhelp" class="btn_nexttoinput ui-btn ui-btn-corner-all ui-btn-b noshadow"><img src="images/icon_help_mark.png"></button>' +
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="#" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_ok') + '</a>' +
        '<a href="#" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back">' + stringres.get('btn_cancel') + '</a>' +
    ...
    // treat serveraddress_user as upperserver. Used in case of standalone tunnel server
            ...
            $('#adialog_positive').on('click', function (event)
            {
                ...
                if (!common.isNull(textBoxVal) && textBoxVal.length > 0)
                {
                    ...
                    mSettValue = mSettValue.replace('udp://', ''); mSettValue = mSettValue.replace('tcp://', '');
                    mSettValue = mSettValue.replace('wss://', ''); mSettValue = mSettValue.replace('ws://', '');
...
function BeforeStart() // handle autoprovisioning
{
    try{
...
/*
    ...
    */
    $(menuId).append( '<li id="' + MENUITEM_HELP + '"><a data-rel="back">' + stringres.get('menu_help') + '...' + '</a></li>' ).listview('refresh');
