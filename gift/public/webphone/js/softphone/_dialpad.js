...
//                || common.isNull(common.GetParameter('upperserver')) || common.GetParameter('upperserver').length <= 0)
    ...
    $( "#page_dialpad" ).keydown(function(e)
    {
    ...
    $("#btn_dp_1").on("tap", function()
    {
        ...
        {
            alert(common.Te());
            //common.ReceivedConferenceInvite('aaa', 'bbb');
            //common.HttpAsyncHandler('getconfroom: returns: OK: conferenceroom: 1234');
            /*webphone_api.getaudiodevicelist(1, function (res)
            {
                alert('AKOS_REC: ' + res);
                //console.log('AKOS_REC: ' + res);
                
                webphone_api.getaudiodevice(1, function (res)
                {
                    alert('AKOS_REC selected: ' + res);
                    //console.log('AKOS_REC: ' + res);
                }); 
            });*/
            
            //alert(webrtcapi_public.GetWebrtcStats());
            //webrtcapi_public.JsObjecSniffer(__o_peerconnection_class, 2);
            
            //var balanceuri = 'http://rtc.mizu-voip.com/mvapireq/?authkey=1568108345&apientry=wsload&brandid=-1&brandname=WebPhone&companyname=&upperserver=voip.mizu-voip.com&authid=9999&now=415';
            //common.UriParser(balanceuri, '', '', '', '', 'wsload');
            
            //common.Wsunreg();
           /*webphone_api.getaudiodevice(0, function (res)
            {
                console.log('AKOS_REC: ' + res);
            });
            ...
            webphone_api.getaudiodevice(1, function (res)
            {
                console.log('AKOS_PLAY: ' + res);
            });*/
            
            //console.log('STUNAKOS: ' + common.GetOneStunSrv());
            
            //common.UploadAutoprov();
//DEBUGPRESENCE   common.StartPresence2();
            
            //var ktoken = 'http://196.1.120.147/CDRTool/mobile.php?action=reg&sim=1449506495826&reg=c458d3c82587601ddce620fb9cbe5eff';
            //common.UriParser(ktoken, "", "", "", "", "kokotalk_reg");
...
function PutNumber(val)
{
    ...
    if (!common.isNull(val) && common.IsNumber(val) && common.IsNumber(nrfield.value))
    {
        PlayDtmfSound(val);
    }
...
var audioElem = null;
function PlayDtmfSound(val)
{
    try{
    if (!window.HTMLAudioElement)
    {
        common.PutToDebugLog(2, 'WARNING, PlayDtmfSound audio html element not supported in this browser: ' + common.GetBrowser());
        return;
    }

    if (common.isNull(val)) { return; }
    
    if (val === '*') { val = 'x'; }
    else if (val === '#') { val = 'y'; }
    

    if (common.isNull(audioElem))
    {
        audioElem = document.getElementById('dtmfsound' + val);
        
        if (common.isNull(audioElem))
        {
            common.PutToDebugLog(2, 'ERROR, PlayDtmfSound cannot get reference to audio element');
            return;
        }
        
        audioElem.play();
    }else // if previous audio element exists, try to stop playback and reset it, before playing the current file
    {
        audioElem.pause();
...
// get current audio element
    ...
    } catch(err) { common.PutToDebugLogException(2, "_dialpad: PlayDtmfSound", err); }
}
...
// don't catch input if a popup is open, because popups can have input boxes, and we won't be able to write into them
    ...
    var charCode = (event.which) ? event.which : event.keyCode; // workaround for firefox
    
    // listen for control key, so we don't catch ctrl+c, ctrl+v
    if ((ctrlDown/* && (charCode == vKey || charCode == cKey)*/)
            || altDown || shiftDown)
    {
        return false;
    }
...
'<div id="quickcall_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + popupWidth + 'px;">' +
    ...
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="#" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_quickcall') + '</a>' +
...
function onStart(event)
{
    ...
    if (!common.isNull(document.getElementById('status_dialpad')))
    {
...
//autoprov: if no voicemail - then fast call: text input number to call
    if (common.GetParameterInt('voicemail', 2) !== 2)
    {
        $('#btn_voicemail_img').attr('src', 'images/btn_call_quick_txt.png');
        $("#btn_voicemail").attr("title", stringres.get("hint_quickcall"));
    }else
    {
        $('#btn_voicemail_img').attr('src', 'images/btn_voicemail_txt.png');
        $("#btn_voicemail").attr("title", stringres.get("hint_voicemail"));
    }
    ...
    if (common.GetConfigInt('brandid', -1) === 50) // Favafone
    {
        $("#btn_message_img").attr("src", "images/icon_recharge_dollar.png");
    }
...
function PopulateListRecents() // :no return value
{
    ...
    if ( common.isNull(global.recentlist) || global.recentlist.length < 1 )
    {
        ...
        $("#btn_showhide_numpad_img").attr("src", 'images/btn_voicemail_txt_big.png');
        $("#btn_showhide_numpad").attr("title", stringres.get("hint_voicemail"));
        return;
    }else
    {
        btn_isvoicemail = false;
        $("#btn_showhide_numpad_img").attr("src", 'images/btn_numpad_txt.png');
        $("#btn_showhide_numpad").attr("title", stringres.get("hint_numpad"));
    ...
    var template = '' +
        '<li data-theme="b"><a id="recentitem_[RCID]" class="ch_anchor mlistitem">' +
            '<div class="item_container">' +
                '<div class="ch_type">' +
                    '<img src="images/[ICON_CALLTYPE].png" />' +
                '</div>' +
...
//Aug, 26 2013 10:55
        ...
        if (enablepres)
        {
            ...
            {
                presenceimg = '<img src="images/presence_grey.png" />';
            }
            else if (presence === '1') // invisible
            {
                presenceimg = '<img src="images/presence_white.png" />';
            }
            else if (presence === '2') // idle
            {
                presenceimg = '<img src="images/presence_yellow.png" />';
            }
            else if (presence === '3') // pending
            {
                presenceimg = '<img src="images/presence_orange.png" />';
            }
            else if (presence === '4') // DND
            {
                presenceimg = '<img src="images/presence_red.png" />';
            }
            else if (presence === '5') // online
            {
                presenceimg = '<img src="images/presence_green.png" />';
            }
            else
            {
                presenceimg = '';
...
//        '<a href="#" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back" data-transition="flow">Delete</a>' +
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="#" id="btn_adialog_ok" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_ok') + '</a>' +
        '<a href="#" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back">' + stringres.get('btn_cancel') + '</a>' +
    '</div>' +
'</div>';
...
'<div id="adialog_videocall" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + popupWidth + 'px;">' +
    ...
    '<div role="main" class="ui-content adialog_content adialog_btn_nexttoinput">' +
        ...
        '<button id="btn_pickct" class="btn_nexttoinput ui-btn ui-btn-corner-all ui-btn-b noshadow"><img src="images/' + btnimage + '"></button>' +
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="#" id="adialog_positive" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back" data-transition="flow">' + stringres.get('btn_videocall') + '</a>' +
        '<a href="#" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_2button" data-rel="back">' + stringres.get('btn_cancel') + '</a>' +
    '</div>' +
'</div>';
