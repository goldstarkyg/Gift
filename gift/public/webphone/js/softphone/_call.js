...
function onStart(event)
{
    ...
    $('#btn_hangup_img').attr('src', 'images/btn_hangup_txt.png');
...
// don't display username and name, if both are the same
    ...
    {
        $('#calledcaller').html(global.callName + '&nbsp;(' + callnumber + ')&nbsp;');
    }else
    {
        $('#calledcaller').html(callnumber);
    }
            ...
            setTimeout(function ()
            {
                        ...
                        && (common.isNull(global.rating) || (global.rating).length < 1) ) // means rating is not received from signaling
                {
                    webphone_api.needratingrequest(function (val) // API_NeedRatingRequest
                    {
                        if (val === true)
                        {
                            common.UriParser(ratinguri, '', callnumber, '', '', 'ratingrequest');
                        }
                    });
                }
            }, 4000);
...
function MeasureCall() // resolve window height size change
{
    ...
    if (calltype === "outgoing")
    {
        pageHeight = pageHeight - $("#hangup_layout").height() - $("#callfunctions_layout").height() - $(".separator_color_bg").height() - 3;
    ...
    if (calltype === "incoming")
    {
        ...
        {
            pageHeight = pageHeight - $("#acceptreject_layout").height() - 5;
            if (volumevisible) { pageHeight = pageHeight - $("#volumecontrols").height(); }
            if (audiodevicevisible) { pageHeight = pageHeight - $("#audiodevice_container").height(); }
            pageHeight = Math.floor(pageHeight);
        }else
...
function HangupCall()
{
    ...
    {
        $('#callfunctions_layout').hide();
        $('#btn_hangup_img').attr('src', 'images/btn_close_txt.png');
        webphone_api.hangup();
    }else
    {
        webphone_api.hangup();
...
// listen for enter onclick, and click OK button
    ...
    $('#adialog_positive').on('click', function (event)
    {
        ...
        if (!common.isNull(textboxval) && textboxval.length > 0)
        {
            ...
            {
                webphone_api.conference(textboxval, -1);
...
function Mute() // :boolean   handle Mute - onClick 
{
    ...
    var muteSuccess = webphone_api.mute(global.muteState, muteDirection, -1);
    ...
    {*/
        ...
        global.muteState = !global.muteState;
        return true;
    /*}*/
...
{
    try{
    var holdSuccess = webphone_api.hold(global.holdState, -1);

    CFUsageClickCount(CLICK_HOLD);

    if (holdSuccess)
    {
        global.holdState = !global.holdState;
        return true;
    }
...
// remove data transition for windows softphone, because it's slow
    ...
    if (featureset > 0 && availableFunc.indexOf('transfer') >= 0)
    {
