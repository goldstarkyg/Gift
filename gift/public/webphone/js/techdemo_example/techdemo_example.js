...
function AcceptRejectPopup(from)
{
    ...
    {
        armodal.close();
        PutToDebugLog(5,"EVENT, live_demo NPDownloadAndInstallSDK Accept onclick");
        Accept();
    });
    ...
    {
        armodal.close();
        PutToDebugLog(5,"EVENT, live_demo NPDownloadAndInstallSDK Reject onclick");
        Reject();
    });
...
{
    try{
    if ( isNull(common_public.settmap) || isNull( common_public.settmap['magicnumber'] ))
    {
...
// callerid, displayname, proxyaddress, webrtcserveraddress
    ...
    document.getElementById('settings_list').innerHTML = content;
