app.controller('EquipmentCreateController', function ($scope, $rootScope, $http, $interval, $uibModal, $stateParams, $httpParamSerializer, AuthService, GuestService, toaster) {
    ...
    $scope.statuses = $http.get('/frontend/equipment/statuslist')
        .then(function(response){
            ...
            $scope.equipment.status_id =  $scope.statuses[0].id;
        });
    ...
    $scope.createEquipment = function(){
        ...
        if($scope.equipment.external_maintenance_company == null) {
            $scope.applyMessage('Please enter external maintenance company!');
            return;
        }
        if($scope.equipment.department == null) {
            $scope.applyMessage('Please enter department!');
            return;
        }
        if($scope.equipment.life == null) {
            $scope.applyMessage('Please enter life!');
            return;
        }
        if($scope.equipment.equipment_group.length == 0) {
            $scope.applyMessage('Please enter equipment group!');
            return;
        }
        if($scope.equipment.part_group.length == 0) {
            $scope.applyMessage('Please enter part group!');
            return;
        }
        if($scope.equipment.location_group_member_id == null) {
            $scope.applyMessage('Please enter location!');
            return;
        }
        if($scope.equipment.status_id == null) {
            $scope.applyMessage('Please enter status!');
            return;
        }
