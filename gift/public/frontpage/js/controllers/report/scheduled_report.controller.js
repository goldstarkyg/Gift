app.controller('ScheduledReportController', function($scope, $rootScope, $http, $httpParamSerializer, $window, $timeout, $uibModal, AuthService, toaster, liveserver) {
    ...
    $scope.applyDetail = function applyDetail(row) {
        ...
        if(frequency == 'Custom Days') detail = 'Every '+date+' at '+time ;
        return detail;
    }
    ...
    $scope.section = '';
    var currentIndex = 0;
    $scope.onSelectRow = function(row,index) {
        $scope.datalist[currentIndex].sel = false;
        currentIndex = index;
        row.sel = true;
        if(row.report_type == 'callaccount')
            $scope.callaccountControl(row);

        if(row.report_type == 'callcenter')
            $scope.callcenterControl(row);

        if(row.report_type == 'callclassify')
            $scope.callclassifyControl(row);

        if(row.report_type == 'guestservice')
            $scope.guestserviceControl(row);

        $scope.section = row.report_type;
    ...
    $scope.callaccountControl = function(row) {
        ...
        $scope.building_id = filter.building_id;
